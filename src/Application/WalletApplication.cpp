...
const char URI_SCHEME_NAME[] = "som";
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("somwallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] SoM wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "SoM wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have somd with non-default RPC port, you should terminate it and relaunch SoMWallet\n"
      "or\n"
      "Set the Local deamon required port in SoMWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
