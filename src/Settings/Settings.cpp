...
const char DEFAULT_WALLET_FILE_NAME[] = "som.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultNodeList << "http://91.214.70.62:12898";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("somwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("somwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("somwallet.desktop");
  ...
  res = autorunSettings.contains("somwallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("somwallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.somovanets.somwallet");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("somwallet", appPath);
    } else {
      autorunSettings.remove("somwallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\SoM", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:SoM");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\SoM\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\SoM\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("somwallet.cfg"));
